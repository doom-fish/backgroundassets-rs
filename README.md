...
> **Status:** v0.2.0 adds the dedicated `async_api` module for executor-agnostic Background Assets futures, alongside `BADownload`, `BAURLDownload`, `BADownloadManager`, `BAAppExtensionInfo`, and a Rust-hosted `BADownloaderExtension` adapter.

## Quick start

```toml
backgroundassets = { version = "0.2", features = ["async"] }
```
...
| Async workflows | `async_api::{AsyncAssetPackManager, AsyncDownloadManager}`, status-update streams, downloader-extension event stream *(requires `async`)* |
